...
useEffect(() => {
  const fetchPostsAndRatings = async () => {
    ...
    if (!error && postData) {
      ...
      // fallback to initial_rating if no user ratings
      for (const post of postData) {
        if (!avgRatings[post.id] && post.initial_rating > 0) {
          avgRatings[post.id] = post.initial_rating
        }
      }
  ...
  fetchPostsAndRatings()
    ...
    return (
        <main className="max-w-7xl mx-auto px-4 py-12 space-y-10">
            <h1 className="text-3xl font-bold text-center">🍽️ Discover Delicious Places</h1>
                        ...
                        >
                            ...
                            <div className="p-4 flex flex-col justify-between flex-grow">
                                <div>
                                    ...
                                    {(ratings[post.id] || post.initial_rating) > 0 && (
                                        <div className="flex items-center gap-1 mb-2">
                                            {[1, 2, 3, 4, 5].map((i) => {
                                                const ratingToShow = ratings[post.id] ?? post.initial_rating
                                                return (
                                                    <Star
                                                        key={i}
                                                        className={`w-4 h-4 ${i <= Math.round(ratingToShow) ? 'text-yellow-500' : 'text-gray-300'}`}
                                                        fill={i <= Math.round(ratingToShow) ? 'currentColor' : 'none'}
                                                    />
                                                )
                                            })}
                                            <span className="text-sm text-gray-500">
                                                ({(ratings[post.id] ?? post.initial_rating).toFixed(1)})
                                            </span>
                                        </div>
                                    )}
